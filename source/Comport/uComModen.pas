...
begin
  ...
  //Global.LogCtrlWriteA6001(FIndex, 'FRecvData : ' + FRecvData);

  if Length(FRecvData) < 16 then
    Exit;

  //Global.LogCtrlWriteA6001(FIndex, 'FRecvData1 : ' + FRecvData);
  if Pos(MODEN_STX, FRecvData) = 0 then
    Exit;

  //Global.LogCtrlWriteA6001(FIndex, 'FRecvData2 : ' + FRecvData);
  if Pos(MODEN_ETX, FRecvData) = 0 then
    Exit;
    //Global.LogCtrlWriteA6001(FIndex, 'FRecvData3 : ' + FRecvData);
  ...
  //Global.LogCtrlWriteA6001(FIndex, 'FRecvData3 : ' + FChannel + ' / ' + Copy(FRecvData, 2, 1));
  ...
  begin
    ...
    rTeeboxInfo.TeeboxNo := Global.Teebox.GetDevicToFloorTeeboxNo(FFloor, FChannel); //타석 번호
    ...
    begin
      sLogMsg := IntToStr(FLastExeCommand) + ' 요청: ' + IntToStr(Global.Teebox.GetDevicToFloorTeeboxNo(FFloor, FChannel)) +
                 ' / ' + Global.Teebox.GetDevicToTeeboxNm(FChannel) + ' / ' + FChannel +
                 ' / 응답: ' + IntToStr(Global.Teebox.GetDevicToFloorTeeboxNo(FFloor, FChannel)) +
                 ' / ' + FRecvData;
...
begin
  rTeeboxInfo.StoreCd := ''; //가맹점 코드
  rTeeboxInfo.TeeboxNo := Global.Teebox.GetDevicToFloorTeeboxNo(FFloor, AChannel); //타석 번호
