...
procedure TTcpThreadHeat.SetHeatuse(ATeeboxNm, AType, AAuto, AStartTm: String; ACtrl: Boolean = False);
var
  nIndex, nHeatNo: Integer;
  sStr, sTeeboxNmTm: String;
begin
  nHeatNo := 0;
  sTeeboxNmTm := ATeeboxNm;

  for nIndex := 1 to FTeeboxLastNo do
  begin
    //if FHeatInfoList[nIndex].TeeboxNm = ATeeboxNm then
    if FHeatInfoList[nIndex].TeeboxNm = sTeeboxNmTm then
    begin
      nHeatNo := nIndex;
      Break;
    end;
  ...
  begin
    ...
    sStr := 'TeeboxNm: ' + sTeeboxNmTm + ' No Device ';
  ...
  sStr := 'TeeboxNm: ' + sTeeboxNmTm + '(' + ATeeboxNm + ')' + ' / ' + AType;
  ...
  if ACtrl = True then
    ...
    SetCmdSendBuffer(sTeeboxNmTm, AType);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Global.SetTeeboxHeatConfig(FHeatInfoList[nIndex].TeeboxNm, '', '0', FHeatInfoList[nIndex].UseAuto, '');
    end;
  end;
...
function TTcpThreadHeat.GetHeatUseStatus(ATeeboxNm: String): String;
var
  nIndex: Integer;
  sStatus: String;
begin
...
procedure TTcpThreadHeat.SetCmdSendBuffer(ATeeboxNm, AType: String);
var
  sSendData: AnsiString;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      SetCmdSendBuffer(FHeatInfoList[nHeatNo].TeeboxNm, '0');
