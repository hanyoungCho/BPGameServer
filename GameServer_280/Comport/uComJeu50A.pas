...
begin
  ...
  Global.Log.LogWriteMulti(FIndex, 'TComThreadJeu50A Create');
...
begin
  ...
  Global.Log.LogWriteMulti(FIndex, 'TComThreadJeu50A ComPortSetting : ' + IntToStr(Index));
end;
...
begin
  ...
  begin
    ...
    begin
      Global.Log.LogReadMulti(FIndex, 'FRecvData fail : 15 수신에러' + IntToStr(FLastExeCommand) + ' : ' +
  ...
  begin
    Global.Log.LogReadMulti(FIndex, '요청타석기: ' + IntToStr(Global.Teebox.GetDevicToTeeboxNo(FChannel)) + ' / 총글자수: ' + IntToStr(Count) + ' / ' + FRecvData);
    FRecvData := '';
    Exit;
  end;
