...
type
  ...
  public
    ...
    function SetLaneAssignClose(AAssignNo: String): String;   //레인배정종료
    ...
    function ChgAssignBowlerMove(AAssignNo, ABowlerId, ATargetLaneNo, AUserId, sTerminalId: String): String; // 볼러 이동
...
begin
  ...
  if nCompetitionSeq > 0 then //취소되는 게임이 대회이면
  begin
    bCompetitionEnd := True;

    for i := 0 to LaneCnt - 1 do
    begin
      if FLaneList[i].Assign.CompetitionSeq = nCompetitionSeq then //대회
      begin
    ...
    begin
      nCompetitionIdx := GetCompetitionIndex(nCompetitionSeq);
      ...
      begin
        ...
        sLog := '대회정보 초기화- 대회코드: ' + IntToStr(nCompetitionSeq);
        Global.Log.LogReserveWrite(sLog);
      end;
...
function TLane.SetLaneAssignClose(AAssignNo: String): String;
var
  nIdx, nCompetitionSeq, i: Integer;
  sLog, sResult: String;
  jSendObj: TJSONObject;
...
begin
  ...
  sLog := '체크아웃 - No: ' + IntToStr(FLaneList[nIdx].LaneNo) + ' / Nm: ' + FLaneList[nIdx].LaneNm + ' / ' + AAssignNo;
  Global.Log.LogReserveWrite(sLog);
...
begin
  ...
  begin
    ...
    else if (FLaneList[nIdx].GameCom.Status = '88') or (FLaneList[nIdx].GameCom.Status = '80') then //종료-게임 전체볼러 모두 종료, 버전이 다른경우 있음
    begin
...
begin
  ...
  begin
    ...
    if FLaneList[nIdx].GameCom.Status = '88' then //종료-게임 전체볼러 모두 종료
    begin
      if FLaneList[nIdx].GameCom.BowlerCnt = 0 then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          bOdd := odd(FLaneList[i].LaneNo); //홀수 여부
          if (bOdd = False) then
          begin
            Global.Com.SendLaneGameNext(FLaneList[i - 1].LaneNo, FLaneList[nIdx].Assign.LeagueYn);
          end;
        end;
...
function TLane.ChgAssignBowlerMove(AAssignNo, ABowlerId, ATargetLaneNo, AUserId, sTerminalId: String): String;
var
  ...
  jSendObj, jSendObj2: TJSONObject;
  //rBowlerInfoTM: TBowlerInfo;
  rHoldInfo: THoldInfo;
  nByte: Byte;
...
begin
  ...
  begin
    ...
    try
      jSendObj := TJSONObject.Create;
      jSendObj.AddPair(TJSONPair.Create('store_cd', Global.Config.StoreCd));
      jSendObj.AddPair(TJSONPair.Create('assign_no', FLaneList[nTIdx].Assign.AssignNo));
      jSendObj.AddPair(TJSONPair.Create('lane_no', FLaneList[nTIdx].LaneNo));
      jSendObj.AddPair(TJSONPair.Create('game_div', FLaneList[nTIdx].Assign.GameDiv));
      jSendObj.AddPair(TJSONPair.Create('game_type', FLaneList[nTIdx].Assign.GameType));
      jSendObj.AddPair(TJSONPair.Create('reserve_datetime', FormatDateTime('YYYY-MM-DD hh:nn:ss', now)));
      jSendObj.AddPair(TJSONPair.Create('user_id', AUserId));
      jSendObj.AddPair(TJSONPair.Create('terminal_id', sTerminalId));
