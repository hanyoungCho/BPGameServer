...
var
  LJobName, LAssignNo, LBowlerId, LBowlerName, LResMsg: string;
...
begin
  ...
  try
    if not BPDM.RelocateBowler(LAssignNo, LBowlerId, FLaneNo2, LResMsg) then
...
begin
  ...
  try
    if not BPDM.RelocateBowler(LAssignNo, LBowlerId, FLaneNo1, LResMsg) then
      raise Exception.Create(LResMsg);
    if not BPDM.ExecuteABSQuery(Format('UPDATE TBSaleItem SET assign_lane_no = %d WHERE assign_lane_no = %d AND bowler_id = %s;',
...
begin
  ...
  try
    ...
    if not BPDM.ExecuteABSQuery(Format('UPDATE TBSaleItem SET assign_lane_no = %d WHERE assign_lane_no = %d;', [FLaneNo2, LAssignLaneNo]), LResMsg) then
      raise Exception.Create(LResMsg);
